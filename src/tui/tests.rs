...
fn dd_cta_edits_in_header_region() {
    ...
    {
        assert_ne!(
            page_cta.parent_class, header_cta.parent_class,
...
fn textarea_expand_click_places_caret() {
    ...
    send_mouse(&mut app, MouseEventKind::Down(MouseButton::Left), 3, 2);
...
fn tier_b_add_item_via_A_key() {
    ...
    let before = match app.modal.as_ref() {
        Some(Modal::FormEdit { state, .. }) => {
            state.sub_state.get("items").map(|v| v.len()).unwrap_or(0)
        }
        _ => panic!("expected FormEdit"),
    };
    send_key(&mut app, KeyCode::Char('A'), KeyModifiers::SHIFT);
...
fn textarea_wraps_long_lines_to_width() {
    let (display, first, total) = render_textarea_display_window("abcdefghij", 0, true, 4, Some(5));
    assert_eq!(first, 0);
    assert_eq!(total, 2);
...
fn pages_panel_r_with_locked_slug_renames_title_only() {
    ...
    assert_eq!(
        app.site.pages[0].slug, orig_slug,
        "locked slug must not regenerate"
    );
}
...
fn f2_shows_warning_status_when_theme_status_is_some() {
    let mut app = App::new(
        ...
        "default".to_string(),
        Some(
            "theme 'foo.yml' declares version 99 (expected 1); using built-in defaults".to_string(),
        ),
    );
    send_key(&mut app, KeyCode::F(2), KeyModifiers::NONE);
...
fn begin_export_flow_with_saved_export_dir_commits_directly() {
    ...
    assert!(
        tmp.join("web").exists(),
        "export directory should have been created"
    );
...
fn load_with_diverging_backup_pushes_info_toast() {
    ...
    let toast = app.toasts.iter().find(|t| {
        t.message
            .to_lowercase()
            .contains("differs from last manual save")
    });
    assert!(
        toast.is_some(),
...
fn header_and_footer_details_hits_align_with_lines() {
    ...
    let decl = header_lines
        ...
        .expect("header decl");
    assert!(
        header_view.hits[decl].is_empty(),
    ...
    assert!(
        ...
        "column decl uses string fallback"
    );
}
...
fn insert_picker_allowed_kinds_match_focused_region() {
    ...
    app.selected_region = SelectedRegion::Header;
    assert!(
        !app.filtered_component_kinds("")
            .contains(&ComponentKind::Hero)
    );
    app.selected_region = SelectedRegion::Page;
    assert!(
        app.filtered_component_kinds("")
    ...
    assert!(
        !app.filtered_component_kinds("")
            .contains(&ComponentKind::HeaderSearch)
    );
