...
#   VERSION=v1.11.0                # default: latest GitHub Release
...
#
# Piped extras:
#   curl -fsSL … | bash -s -- --version v1.11.0
...
Override defaults via env vars:
  VERSION=v1.11.0                pin a release (default: latest)
  PREFIX=$HOME/.local           binary lives at $PREFIX/bin/dd_siteforge
...
normalize_version() {
    # Empty → latest. Accept "1.11.0" or "v1.11.0".
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --from-release|--binary)
                            mode="release"; shift ;;
        --version)
            [ $# -ge 2 ] || { red "--version needs a value (e.g. v1.11.0)"; exit 1; }
            VERSION="$2"
            shift 2
            ;;
